# app/background/job_handlers.py
...
async def handle_supplier_ingest(uow: UoW, payload: dict[str, Any]) -> None:
    ...
    if not id_supplier:
        raise InvalidArgument("supplier_ingest job without id_supplier")
...
async def dispatch_job(kind: str, payload_json: str, uow: UoW) -> None:
    ...
    if not handler:
        raise InvalidArgument(f"No job handler registered for kind={kind}")
