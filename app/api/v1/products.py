...
from app.domains.catalog.usecases.products.command.add_prestashop_match import (
    execute as uc_add_prestashop_match,
)
...
from app.schemas.products import (
    ...
    ProductImportOut,
    BulkImportIn,
    BulkImportOut, AddPrestashopMatchIn,
)
...
def get_product_facets(
        ...
        gtin: str | None = Query(None),
        partnumber: str | None = Query(None),
        id_brand: int | None = Query(
            None, description="Filtrar inicialmente por id_brand (opcional)"),
...
@router.post(
    "/add_prestashop_match",
    response_model=ProductDetailOut,
    summary="Verifica se GTIN esta registado no sistema, caso esteja cria associação"
)
def add_prestahop_match(
        uow: UowDep,
        payload: AddPrestashopMatchIn,
) -> ProductDetailOut:
    return uc_add_prestashop_match(
        uow,
        gtin=payload.gtin,
        id_product_ecommerce=payload.id_product_ecommerce,
